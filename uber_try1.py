...
def compare_time(to_check):
    ...
    to_check = "10-11-1999:12-12-12"
    epoch1 = int(time.mktime(time.strptime(current,pattern)))
    epoch2 = int(time.mktime(time.strptime(to_check,pattern)))
    if epoch2 > epoch1:
        return 1
    return 0
...
def add_user():
    if (request.method == 'PUT'):
        ...
        if ( match == None):
            ...
            d["data"] = dataDict
            d["check"] = "user"
            found = requests.post("http://localhost:5000/api/v1/db/read", data = json.dumps(d))
            found = found.text
            found = int(found)
            if (found == 1):
                return {}, 400  #username already exists
            else:
                retjson = requests.post("http://localhost:5000/api/v1/db/write", data = json.dumps(d))
...
def new_ride():
    if (request.method == 'POST'):
        ...
        if found == 0:
            ...
            # d["rideid"] = 1
            d["check"] = None
            rideid = requests.post("http://localhost:5000/api/v1/db/read", data = json.dumps(d))
            rideid = rideid.text
            rideid = int(rideid)
...
def upcoming_rides():
    if (request.method == 'GET'):
        ...
        if (source >= 1 and source <= 198) and (destination >= 1 and destination <= 198) and (source != destination):
            d = {}
            d["table"] = "rides"
            d["work"] = "GET_RIDES"
            ...
            d["data"] = dataDict
            d["check"] = None
            dbquery = requests.post("http://localhost:5000/api/v1/db/read", data = json.dumps(d))
